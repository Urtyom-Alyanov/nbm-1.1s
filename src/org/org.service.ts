...
export class OrgService {
  ...
  async create(user: UserEntity, data: CreateInputOrg) {
    ...
    const { item: Country } = await this.countryService.findOne({id: data.cId}, ["user"]);
    if(Country) {
      if(Country.onlyGov && Country.user !== user) throw new BadRequestException("У данного государства стоит знак \"Только гос-венные организации\"");
      Org.countr = Country;
    }
    Org.user = user;
    ...
    Org.desc = data.text;
    Org.img = data.img;
    if(data.isPublished && user.levelAccess > 1) Org.isPublished = data.isPublished;
    await this.orgRepository.save(Org);
    return { item: Org };
  };

  async findOne(data: FindOneOrg, relations?: string[], isPublished: boolean | "none" = true) {
  ...
  async findMany({limit, page, ...data}: FindAllOrg, relations?: string[], isPublished: boolean | "none" = true) {
    let where: FindConditions<OrgEntity>;
  ...
  async update(user: UserEntity, data: EditInputOrg) {
    ...
    const { item: Country } = await this.countryService.findOne({id: data.cId}, ["user"]);
    if(user.levelAccess < 2 && Org.user !== user) throw new BadRequestException("Неверный пользователь");
    if(Country) {
      if(Country.onlyGov && Country.user !== user) throw new BadRequestException("У данного государства стоит знак \"Только гос-венные организации\"");
      Org.countr = Country;
    }
  ...
  async pay(user: UserEntity, data: PayInput) {
    ...
    if(user !== Org.user) await pubEvent({
      forId: Org.user.id,
      isCool: true,
      text: `Вашу организацию (${Org.name}) спонсировал ${user.nick ? user.nick : user.username}.`,
  ...
  async unpay(user: UserEntity, data: PayInput) {
    ...
    if(user.levelAccess < 3 && Org.user !== user) throw new BadRequestException("Неверный пользователь");
    user.balance -= data.summ;
    Org.balance += data.summ;
    await this.orgRepository.save(Org);
    await this.usersService.save([user]);
