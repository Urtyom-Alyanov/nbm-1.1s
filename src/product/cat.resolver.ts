...
export class CatResolver {
  ...
  @Query(() => ItemCatModel)
  async findOneCategory(@Args() data: FindOneCat): Promise<ItemCatModel> {
    return await this.catService.getOne(data);
  }

  @Query(() => ManyCatModel)
  async findManyCategory(@Args() data: FindAll): Promise<ManyCatModel> {
  ...
  @Mutation(() => ItemCatModel)
  @ForAuth(3)
  async deleteCategory(@Args('id') id: number): Promise<StatusModel> {
    return await this.catService.delete(id);
  }

  @ResolveField(() => [ProductModel])
  async products(@Parent() category: CatModel): Promise<ProductModel[]> {
