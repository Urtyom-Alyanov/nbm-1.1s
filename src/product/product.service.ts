...
export class ProductService {
  ...
  async getOne(data: FindOneProduct, isPublished: boolean | "none" = true, relations?: string[]) {
  ...
  async getMany({limit, page, ...data}: FindManyProduct, isPublished: boolean | "none" = true, relations?: string[]) {
    let where: FindConditions<ProductEntity>;
  ...
  async create(user: UserEntity, data: CreateInputProduct) {
    const { item: Org } = await this.orgService.findOne({ id: data.oId }, ["user"]);
    if(user !== Org.user && user.levelAccess < 3) throw new BadRequestException("Неверный пользователь");
    ...
    Product.category = data.catId ? (await this.categoryService.getOne({ id: data.catId })).item : null;
    Product = await this.productRepository.save(Product);
    return { item: Product }
  };
  async update(user: UserEntity, data: EditInputProduct) {
    let Product = (await this.getOne({ id: data.id }, true, ["org", "org.user"])).item;
    if(user !== Product.org.user && user.levelAccess < 3) throw new BadRequestException("Неверный пользователь");
    Product.name = data.name;
    Product.desc = data.text;
    Product.sale = data.sale;
    Product.img = data.img;
  ...
  async delete(user: UserEntity, id: number) {
    const { item: Product } = await this.getOne({ id }, false, ["org", "org.user"]);
    if(user !== Product.org.user && user.levelAccess < 3) throw new BadRequestException("Неверный пользователь");
    await this.productRepository.remove(Product);
    return { isOk: true }
  };
  async addItems(user: UserEntity, data: PayInput) {
    const { item: Product } = await this.getOne({ id: data.id }, false, ["org", "org.user"]);
    if(user !== Product.org.user && user.levelAccess < 3) throw new BadRequestException("Неверный пользователь");
  ...
  async decline(data: PayInput) {
    const { item: Product } = await this.getOne({ id: data.id }, false, ["user"]);
    await pubEvent({
      forId: Product.org.user.id,
      isCool: false,
