...
export class CartService {
  ...
  async payProduct(user: UserEntity, data: PayInput) {
      const {item: Product} = await this.productService.getOne({ id: data.id }, true, ["org", "org.user"]);
      if(Product.productType && Product.count < data.summ) throw new BadRequestException("На складе нет столько товаров, скольно вы хотите. Подождите закупки.");
      user.balance -= (data.summ * Product.sale) * 1.05;
      Product.org.balance += data.summ * Product.sale;
      this.orgService.save([Product.org]);
      this.userService.save([user]);
      let sale = (await this.findAllSales({ userId: user.id, productId: data.id })).items[0];
      if(!sale) {
        sale = new CartEntity();
      ...
      await pubEvent({
        ...
        isCool: true,
        text: `Ваш продукт (${Product.name}, комп. ${Product.org.name}) купил ${user.nick ? user.nick : user.username}.`,
