...
import { ItemProductModel, ManyProductModel, ProductModel, SaleModel } from "./models/product.model";
...
export class ProductResolver {
    ...
    async findAllProduct(@Args() args: FindManyProduct): Promise<ManyProductModel> {
        return this.productService.getMany(args);
    }

    @Query(() => ManyProductModel)
    @ForAuth(2)
    async findNotAcceptedProduct(@Args() args: FindManyProduct): Promise<ManyProductModel> {
        return this.productService.getMany(args, false);
    }

    @Query(() => ItemProductModel)
    async findOneProduct(@Args() args: FindOneProduct): Promise<ItemProductModel> {
    ...
    async sales(
        ...
        return (await this.cartService.findAllSales({ productId: product.id })).items;
    };
